            ...
            fetch(`${baseUrl}/pokemon/${pokemon.toLocaleLowerCase()}`)
                ...
                .then(data => {
                    ...
                    fetch(`${baseUrl}/pokemon-species/${data.id}`)
                        .then(response => response.json())
                        .then(data => {
                            let aboutText = data.flavor_text_entries[getRandomIndex(data.flavor_text_entries)].flavor_text
                    ...
                    fetch(`${baseUrl}/characteristic/${data.id}`)
                        ...
                        .catch(error => {
                            charac.innerHTML = `O ${toUpper(data.name)} não tem característica!`;
                            console.error(`A característica do Pokemón "${data.name}" não existe.`);
                        })
